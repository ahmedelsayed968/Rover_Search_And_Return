...
def avg_dist_angle(x_pixel, y_pixel):
    # Convert (x_pixel, y_pixel) to (distance, angle)
    # in polar coordinates in rover space
    # Calculate distance to each pixel

    dist = np.sqrt(x_pixel ** 2 + y_pixel ** 2)
    avg_dist = np.median(dist)
    # Calculate angle away from vertical for each pixel
    angles = np.arctan2(y_pixel, x_pixel)
    avg_angle = np.median(angles)
    try:
        avg_angle = math.ceil(avg_angle)
    except:
        pass

    try:
        avg_dist = math.ceil(avg_dist)
    except:
        pass
    return avg_dist, avg_angle


def avg_cart_coords(x_pixel, y_pixel):
    x_avg = np.median(x_pixel)
    y_avg = np.median(y_pixel)
    return x_avg, y_avg
...
def perception_step(Rover):
    ...
    if (Rover.pitch < 1 or Rover.pitch > 359) and (Rover.roll < 1 or Rover.roll > 359):
        ...
        dist_obstacle, angle_obstacle = avg_dist_angle(obstacle_x_world, obstacle_y_world)
        x_obs_cart, y_obs_cart = avg_cart_coords(obstacle_x_world, obstacle_y_world)

        dist_rocks, angle_rocks = avg_dist_angle(rock_x_world, rock_y_world)
        x_rocks_cart, y_rocks_cart = avg_cart_coords(rock_x_world, rock_y_world)

        dist_nav, angle_nav = avg_dist_angle(navigable_x_world, navigable_y_world)
        x_nav_cart, y_nav_cart = avg_cart_coords(navigable_x_world, navigable_y_world)

        if (dist_obstacle, angle_obstacle) not in Rover.memory_obstacles \
        ...
        if (dist_rocks, angle_rocks) not in Rover.memory_rocks \
        ...
        if (dist_nav, angle_nav) not in Rover.memory_nav \
