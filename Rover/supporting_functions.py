...
def convert_to_float(string_to_convert):
    if ',' in string_to_convert:
        float_value = np.float64(string_to_convert.replace(',', '.'))
    else:
        float_value = np.float64(string_to_convert)
    return float_value
...
def update_rover(Rover, data):
    ...
    #   Rover.intial_x = Rover.pos[0]
    #   Rover.intial_y = Rover.pos[1]
    ...
    # Near sample flag
    Rover.near_sample = np.int64(data["near_sample"])
    # Picking up flag
    ...
    print('speed =', Rover.vel, 'position =', Rover.pos, 'throttle =',
          ...
          'samples collected:', Rover.samples_collected, 'total time', Rover.total_time)
    # Get the current image from the center camera of the rover
    imgString = data["image"]
    image = Image.open(BytesIO(base64.b64decode(imgString)))
    Rover.img = np.asarray(image)

    print('called -' + str(Rover.total_time))

    if Rover.total_time == 0:
        Rover.initial_x = Rover.pos[0]
        Rover.initial_y = Rover.pos[1]
        print(str(Rover.initial_x) + " - " + str(Rover.initial_y))
...
def create_output_images(Rover):
    ...
    tot_nav_pix = np.float64(len((plotmap[:, :, 2].nonzero()[0])))
    # Next figure out how many of those correspond to ground truth pixels
    good_nav_pix = np.float64(len(((plotmap[:, :, 2] > 0) & (Rover.ground_truth[:, :, 1] > 0)).nonzero()[0]))
    # Next find how many do not correspond to ground truth pixels
    bad_nav_pix = np.float64(len(((plotmap[:, :, 2] > 0) & (Rover.ground_truth[:, :, 1] == 0)).nonzero()[0]))
    # Grab the total number of map pixels
    tot_map_pix = np.float64(len((Rover.ground_truth[:, :, 1].nonzero()[0])))
    ...
    cv2.putText(map_add, "  Collected: " + str(Rover.samples_collected), (0, 85),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    # cv2.putText(map_add," Current Position: "+str(Rover.pos), (0, 110),
    #              cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    # cv2.putText(map_add,"  Initial x: "+str(Rover.initial_x), (0, 130),
    #              cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    # cv2.putText(map_add,"  Initial y: "+str(Rover.initial_y), (0, 150),
    #             cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
